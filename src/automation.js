...
function jekyllWarnings(files) {
  if (!files["_config.yml"]) {
    return;
  }
  if (
    files["_config.yml"] &&
    files["package.json"] &&
    files["package.json"].match(/hexo/)
  ) {
    return;
  }
  if (!files["Gemfile"]) {
    return messages["jekyll with no gemfile"];
  }
  if (!files["Gemfile"].match(/('jekyll'|"jekyll")/)) {
    return messages["jekyll not in gemfile"];
  }
}
...
function packageWarnings(files) {
  ...
  try {
    ...
    return messages["error parsing package.json"];
  }
...
const messages = {
  "jekyll with no gemfile": `It looks like this a **Jekyll** site, but you don't have a **Gemfile**.
Add a Gemfile with Jekyll to make netlify install the right version of Jekyll
before running your build.`,

  "jekyll not in gemfile": `It looks like this is a **Jekyll** site, but your **Gemfile** doesn't list
Jekyll as a dependency. Make sure you add **gem "jekyll"** to your Gemfile
and then run **bundle install**`,

  "roots not in package.json": `It looks like this is a **Roots** project, but you don't have roots
as a dependency in your **package.json**.
Run **npm install roots --save** to make sure netlify installs roots
as a dependency before running your build.`,

  "error parsing package.json": `You have a malformed package.json in your folder. This will most likely lead to a
