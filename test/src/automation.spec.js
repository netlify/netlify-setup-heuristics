...
describe("automation", () => {
  describe("warning", () => {
    ...
    it("should warn about a jekyll site without jekyll in the Gemfile", () => {
      expect(warning({ "_config.yml": true, Gemfile: "middleman" })).toMatch(
        /Gemfile/
      );
    });

    it("should not warn about a well configured jekyll site", () => {
      expect(warning({ "_config.yml": true, Gemfile: 'gem "jekyll"' })).toBe(
        undefined
      );
    });
    ...
    it("should warn about roots without a roots dependency", () => {
      expect(warning({ "package.json": "{}", "app.coffee": true })).toMatch(
        /roots/
      );
    });

    it("should not give jekyll warnings for a hexo site", () => {
      expect(
        warning({
          "_config.yml": true,
          "package.json": '{"dependencies": {"hexo": "^3.7.1"}}'
        })
      ).toBe(undefined);
    });
  });
  ...
  describe("settings", () => {
    ...
    it("should guess a typical roots setup", () => {
      expect(
        settings({ "package.json": '{"dependencies": {"roots": "3.1.0"}}' })
      ).toEqual({
        cmd: "roots compile",
        dir: "public/"
      });
    ...
    it("should guess a create-react-app setup", () => {
      expect(
        settings({
          "package.json": '{"dependencies": {"react-scripts": "1.1.4"}}'
        })
      ).toEqual({
        cmd: "npm run build",
        dir: "build/"
